    ...
    <div className="space-y-4">
      ...
      <Card className="overflow-hidden">
        <div className="bg-gradient-to-br from-gold-500/15 to-transparent p-6">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              <h1 className="font-serif text-2xl font-semibold tracking-tight">
                {isNewUser ? 'Willkommen!' : due > 0 ? 'Time to review' : 'Ready when you are'}
              </h1>
            ...
            <div className="flex gap-2">
              <Link to={due > 0 ? '/practice?start=due' : '/practice?start=mixed'}>
                <Button variant="primary" size="lg">
                  {isNewUser ? 'Start learning' : due > 0 ? `Review ${due}` : 'Practise'}
                </Button>
              </Link>
            </div>
          </div>
          ...
          <div className="mt-6 grid grid-cols-2 gap-5 sm:grid-cols-4">
            <Stat label="Due now" value={due} tone={due > 0 ? 'gold' : undefined} />
            <Stat label="Streak" value={`${streak} d`} tone={streak > 0 ? 'green' : undefined} />
            <Stat
              label="Today"
              value={`${todayXp} XP`}
      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <Card className="p-5">
          <div className="flex items-baseline justify-between">
            ...
            <span className="text-xs tabular-nums text-muted">{gamification.xp} XP</span>
          </div>
          <div className="mt-2 flex items-baseline gap-2">
            <span className="font-serif text-xl font-semibold">{rank.name}</span>
            <span className="text-xs text-muted">
              Level {rank.level} · {rank.english}
            </span>
          </div>
          <ProgressBar value={rankProgress(gamification.xp)} className="mt-3" label="Rank progress" />
        ...
        <Card className="p-5">
          ...
          <p className="mt-1 text-xs text-muted">
            {mastery.total} of {practisable.toLocaleString()} enabled forms seen
          </p>
          ...
          <div className="mt-3 flex h-3 overflow-hidden rounded-full bg-ink-200 dark:bg-ink-800">
            {(
              ...
              <div
                key={key}
                className={colour}
                style={{ width: `${practisable === 0 ? 0 : (count / practisable) * 100}%` }}
                title={`${key}: ${count}`}
              />
            ))}
          </div>
          ...
          <div className="mt-3 flex flex-wrap gap-x-4 gap-y-1 text-xs">
            <LegendDot colour="bg-emerald-500" label="Mastered" value={mastery.mastered} />
            <LegendDot colour="bg-sky-500" label="Review" value={mastery.review} />
            <LegendDot colour="bg-gold-500" label="Learning" value={mastery.learning} />
            <LegendDot colour="bg-red-500" label="Lapsed" value={mastery.lapsed} />
          </div>
        </Card>
      </div>
      ...
      <Card className="p-5">
        ...
        <div className="mt-4 flex h-24 items-end gap-1">
          {last14.map((entry) => (
            <div key={entry.day} className="group flex flex-1 flex-col items-center gap-1">
              <div
                className={cx(
      ...
      {Object.keys(gamification.unlocked).length > 0 && (
        <Card className="p-5">
          <div className="flex items-baseline justify-between">
            ...
            <Link to="/achievements" className="text-xs text-gold-600 hover:underline dark:text-gold-400">
              See all →
            </Link>
          </div>
